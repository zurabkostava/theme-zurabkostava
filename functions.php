...
/* ============================================================
   ZK CUSTOM SEO ENGINE (Stage 3 - Ultimate Architecture)
   ============================================================ */
...
// 2. Custom Rewrite Rules for Sitemap and AI.txt
function zk_seo_rewrite_rules() {
    add_rewrite_rule('^sitemap\.xml$', 'index.php?zk_sitemap=1', 'top');
    add_rewrite_rule('^ai\.txt$', 'index.php?zk_aitxt=1', 'top');
}
...
add_filter('query_vars', 'zk_seo_query_vars');

// 3. Intercept Requests and Output Data
function zk_seo_intercept_requests() {
    if (get_query_var('zk_sitemap')) {
        zk_render_sitemap();
        exit;
    }
    if (get_query_var('zk_aitxt')) {
        zk_render_aitxt();
        exit;
    }
}
add_action('template_redirect', 'zk_seo_intercept_requests', 0);

function zk_render_sitemap() {
    ...
    // Add explicit static/custom routes
    $custom_routes = ['/', '/music/', '/encrolib/', '/contact/'];
    foreach ($custom_routes as $route) {
        echo "  <url>\n";
        echo "    <loc>" . esc_url(home_url($route)) . "</loc>\n";
