            ...
            if ('serviceWorker' in navigator) {
                try {
                    ...
                    await reg.showNotification('WordEvo · Test', {
                        ...
                        icon: window.WORDEVO_ASSET_PATH + '/icons/notification-w-192.png',
                        badge: window.WORDEVO_ASSET_PATH + '/icons/notification-w-96.png',
                        tag: 'wordevo-test',
                        renotify: true,
                        vibrate: [200, 100, 200],
...
async function showNotificationWithCard(notif) {
    ...
    if ('serviceWorker' in navigator) {
        try {
            ...
            if ('Notification' in window && Notification.permission === 'granted') {
                new Notification(title, { body, icon: window.WORDEVO_ASSET_PATH + '/icons/notification-w-192.png', tag: `wordevo-${notif.id || 'reminder'}` });
            }
        }
